...
# ----------------------------
SYSTEM_NAME="NAS-MOVE"
SERVICE_NAME="MoveMonthlyArchivesExample"
...
# ----------------------------
LOG_FILE="/mnt/logs/MoveMonthlyArchivesExample/$(date +'%Y%m%d')_move_monthly_archives.log"
LOCK_FILE="/tmp/monthly_archive_move_example.lock"
...
DST="/mnt/long_term_storage/monthly_backups"
...
# for this workflow.
source "$BACKUP_LIB_DIR/move_backups.sh"
...
#
# This example:
# 1. Moves the selected backup folders
#    from the archive location to
#    long-term storage.
# 2. Verifies successful movement.
# 3. Removes the original folders only
...
# ----------------------------
run_job() {
    log_event "INFO" "START" "script" "0" "SUCCESS" "starting archive move workflow"
    log_event "INFO" "MODE" "script" "0" "SUCCESS" "EXECUTION_MODE=$EXECUTION_MODE"


    # Execute the shared move workflow.
    # Folder discovery, verification,
    # cleanup, retries, and logging are
    # handled internally by move_backups().
    move_backups "$SRC" "$DST"
    local rc=$?

    if [ "$rc" -eq 0 ]; then
        log_event "INFO" "SUMMARY" "workflow" "MOVE=SUCCESS" "SUCCESS" "archive move workflow complete"
    else
        log_event "ERROR" "SUMMARY" "workflow" "MOVE=FAIL" "FAIL" "archive move workflow completed with failures"
    fi
